...
# Convert to binary classification (above/below median = High/Low progression)
median_val = np.median(data.target)
y_binary = (data.target > median_val).astype(int)
df['progression_class'] = y_binary
...
print(f"Classes       : Low Progression (0), High Progression (1)")
print(f"Note          : Original data converted to binary classes")
print(f"                based on median disease progression value")
print("\nFirst 10 Rows of Dataset:")
print("-" * 60)
print(df[['age', 'bmi', 'bp', 's1', 'progression_class']].head(10).to_string(index=False))
print("=" * 60)
...
Description:
  ...
  Here it predicts whether a patient has Low or High diabetes
  disease progression based on medical measurements.
""")
...
Description:
  ...
  of yes/no questions about the patient's medical data like
  age, BMI, blood pressure and follows a path to classify
  whether the patient has Low or High disease progression.
  Easy to understand but can overfit if tree is too deep.
""")
...
Description:
  ...
  For example if K=3 and 2 out of 3 nearest patients have
  High progression, the new patient is also classified as High.
  It does not learn a model, it just memorizes training data
  and compares at prediction time.
""")
...
Description:
  ...
  It finds the best linear relationship between the patient's
  medical measurements and disease progression class.
  It is the simplest algorithm and works well for linear data.
""")
...
Description:
  ...
  and assigns each patient to the nearest center.
